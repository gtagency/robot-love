...
def dif(a, b, threshold):
	return max(abs(int(a) - int(b)) - threshold, 0)
...
def weight(p1, p2): 
    ...
    if ((not p2[qs["gender"]] in orientations[p1[qs["orientation"]]][gender]) and (not "Other" in orientations[p1[qs["orientation"]]][gender])):
        t+=basicallyinfinity
            #        1  2  3   4    5    6    7
    scales = ["NaN", 0, 1, 5, 20, 80, 1000, basicallyinfinity]
    ...
    #how much time
    t+=dif(p1[qs["time"]],p2[qs["time"]],0) * 3
    ...
    #Child being gay
    t+=dif(p1[qs["childgay"]],p2[qs["childgay"]],0) * 5
    #Sex
    ...
    t+=dif(p1[qs["emotionalvulnerability"]],p2[qs["emotionalvulnerability"]],0) * 4
    #do nothing
